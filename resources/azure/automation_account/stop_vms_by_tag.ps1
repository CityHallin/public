...
Foreach ($vm in $vms) {    
    If (($vm.PowerState -eq "VM running") -or ($vm.PowerState -eq "VM stopped")) {
        ...
        Write-Output "VM=$($vm.Name) Current state=$($vm.PowerState). Sending shutdown signal to VM now"
    }
    Elseif ($vm.PowerState -eq "VM deallocated") {            
        Write-Output "VM=$($vm.Name) Current state=$($vm.PowerState). VM already shutdown. Skipping VM"
    }
    Else {              
